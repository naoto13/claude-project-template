---
name: skill-amend
description: スキル観測データや診断結果に基づき、既存 Skill やルールを改善する時に使用する。
---
...
1. `.Codex/state/skill-observations.jsonl` を読み取る
2. `.Codex/state/skill-amendments.jsonl` を読み取る
...
```bash
cp .Codex/skills/{skill-name}/SKILL.md .Codex/skills/{skill-name}/SKILL.md.bak
# or
cp .Codex/rules/{rule-name}.md .Codex/rules/{rule-name}.md.bak
```
...
1. SKILL.md を更新する
2. `.Codex/state/skill-amendments.jsonl` に追記:
