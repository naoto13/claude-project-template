...
cmux 環境で Codex サブエージェントを別ペインに起動し、タスクの実行過程をユーザーに見える形で実行する。
...
```bash
cmux send --surface $SURF "Codex --dangerously-skip-permissions\n"
```
...
> **注意**: `tail -8` で末尾を広めに取る。Codex の出力にはステータスバー等が含まれるため `tail -3` では不足する場合がある。
...
# 2. それぞれ Codex を起動（Trust 処理は省略 — 同パターン）
cmux send --surface $SURF1 "Codex --dangerously-skip-permissions\n"
cmux send --surface $SURF2 "Codex --dangerously-skip-permissions\n"
